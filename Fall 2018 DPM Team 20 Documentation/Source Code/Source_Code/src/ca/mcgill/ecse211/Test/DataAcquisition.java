 ...
 *
 */
 class DataAcquisition implements TimerListener {
	// Class Constants
	...
	static Port portColor1 = LocalEV3.get().getPort("S3");

	// Attach instances of Color and Touch sensors to specified ports.

	static SensorModes myColor1 = new EV3ColorSensor(portColor1);

	// Get an instance of a sample provider for each sensor. Operating
	// modes are specified in the constructor. Note that the color
	// sensor is set to return the intensity of the reflected light.

	static SampleProvider myColorSample1 = myColor.getMode("Red");

	// Need to allocate buffers for each sensor
	static float[] sampleColor1 = new float[myColor.sampleSize()];
	// Entry point
	...
	public void timedOut() {
		...
		myColorSample1.fetchSample(sampleColor1, 0);
		numSamples++;
		System.out.println(numSamples + ", " + sampleColor[0] * 1000 + "        " + sampleColor1[0] * 1000);
	}
}